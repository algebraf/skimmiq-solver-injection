...
const runSequenceBtn = document.querySelector("#skimmiqRunSequenceBtn");
...
runSequenceBtn.addEventListener("click", () => runPastedSequence());
...
async function runPastedSequence() {
  if (busy) return;
  let moves;
  try {
    moves = parseMoveSequence(moveLog.value);
  } catch (error) {
    solverLabel.textContent = "Sequence error";
    setSolverProgress("Invalid move", error instanceof Error ? error.message : String(error));
    return;
  }

  if (!moves.length) {
    solverLabel.textContent = "Sequence empty";
    setSolverProgress("Paste moves", "0 moves");
    return;
  }
  ...
  lastMove = null;
  renderMoveLog();
  await runSequence(moves, `${moves.length} moves`, {
    finalStatus: "Ready",
    finalProgress: { text: "Sequence applied", meta: `${moves.length} moves` }
  });
}
...
async function runSolver() {
  ...
  try {
    ...
    setSolverProgress("Solver error", message.slice(0, 160));
  } finally {
    busy = false;
    setControlsDisabled(false);
    updateLabels();
...
function parseMoveSequence(text) {
  ...
  return tokens.map((token) => {
    try {
      const move = puzzle.getMove(token);
...
function renderMoveLog(prefix = "") {
  if (prefix) {
    moveLog.value = prefix;
    return;
  }
  moveLog.value = history.length ? skimmiqMovesToString(history) : "";
}
...
function solverMethodLabel(method) {
  ...
  if (method === "native-mitm") return "SkimmIQ native MITM";
  if (method === "native-macro") return "SkimmIQ native macros";
  if (method === "native-none") return "Already solved";
  if (String(method || "").startsWith("native-")) return "SkimmIQ native";
  return "Rust WASM";
}

function formatSolverProgress(progress) {
  if (progress.phase === "native-solve") {
    return {
      label: "Native",
      detail: "SkimmIQ solver",
